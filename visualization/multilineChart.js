...
d3.json("../dataset/players_data.json").then(function (data) {
  ...
  x.domain(
    d3.extent(allSeasons, function (d) {
      return new Date(d);
    })
  );
  ...
  let myG = svg.append("g").attr("clip-path", "url(#areaClip)");

  let lineGroup = myG.append("g").attr("class", "line-group");

  let legendGroup = svg.append("g").attr("class", "legend-group");

  svg
    .append("g")
    .attr("class", "eje-x")
    .attr("id", "x-axis-multi")
  ...
  svg
    .append("g")
    .attr("class", "eje-y")
    .attr("id", "y-axis-multi")
    .call(d3.axisLeft(y));

  svg
    .append("text")
    .attr("id", "x-axis-label-multi")
    .attr(
      "transform",
      "translate(" + width / 2 + " ," + (height + margin.top - 20 + 60) + ")"
    )
  ...
  document.getElementById("playButton").addEventListener("click", function () {
    let lines = lineGroup.selectAll(".line").data(processedData, function (d) {
      return d.name;
    });
    ...
    legends
      ...
      .then(() => {
        ...
        let transformacionZoom = d3.zoomIdentity
          .scale(escalaZoom)
        ...
        svg
          .transition()
          .duration(2000)
          .call(zoom.transform, transformacionZoom);
        legendGroup.selectAll(".leyenda").style("font", "5px sans-serif").style("font-family",'Kanit, sans-serif');
      });
